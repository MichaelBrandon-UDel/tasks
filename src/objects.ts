...
export function isValid(question: Question, answer: string): boolean {
    if (question.type === "multiple_choice_question") {
        ...
        const findAnswer = selection.some((q: string): boolean => q === answer);
        if (findAnswer) {
            return true;
        } else {
            return false;
...
export function toShortForm(question: Question): string {
    const qSize = question.name.length;
    if (qSize > 10) {
        const query = question.name.substr(0, 10);
        return question.id + ": " + query;
    } else {
